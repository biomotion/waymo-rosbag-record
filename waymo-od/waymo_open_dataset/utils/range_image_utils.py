...
def compute_range_image_polar(range_image,
  ...
  with tf.name_scope('ComputeRangeImagePolar'):
...
def compute_range_image_cartesian(range_image_polar,
  ...
  with tf.name_scope('ComputeRangeImageCartesian'):
    ...
    if pixel_pose is not None:
      ...
      # [B, 4, 4]
      world_to_vehicle = tf.compat.v1.matrix_inverse(frame_pose)
...
def build_camera_depth_image(range_image_cartesian,
  ...
  with tf.name_scope('BuildCameraDepthImage'):
    # [B, 4, 4]
...
def build_range_image_from_point_cloud(points_vehicle_frame,
  ...
  with tf.name_scope('BuildRangeImageFromPointCloud'):
...
def extract_point_cloud_from_range_image(range_image,
  ...
  with tf.name_scope('ExtractPointCloudFromRangeImage'):
...
def compute_inclination(inclination_range, height, scope=None):
  ...
  with tf.name_scope('ComputeInclination'):
